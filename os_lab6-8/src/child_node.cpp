...
int main(int argc, char *argv[]) {
    ...
    left_socket.setsockopt(ZMQ_RCVTIMEO, 1000);
    string adr_left = "tcp://127.0.0.1:300";
    zmq::socket_t right_socket(context_r, ZMQ_REP);
    right_socket.setsockopt(ZMQ_RCVTIMEO, 1000);
        ...
        if (command == "exec") { //----------EXEC----------
            ...
            if (id_proc == id) { // id == proc_id, this node must execute
                ...
                if (pattern.size() <= text.size()) {
                    ...
                    cout << "Pattern is shorter than text\n";
                }
                if (flag == 0) {
                    res = res + "-1";
                }
            ...
            if (id_proc == id) {
                ...
                if (right_id == 0) { // there is no right node
                    right_id = id_proc;
                    ...
                    if (f == 0) {
                        ...
                        string recieved_message_from_node(static_cast<char *>(message_from_node.data()),
